...
Instrucciones de proyecto para agentes de código (Claude Code, Antigravity, Jules, etc.) al trabajar en **Skandia Intelligence App**.

Este archivo es la fuente única de verdad, compartida entre herramientas:
- **Claude Code** lo carga vía el import `@AGENTS.md` en `CLAUDE.md`.
- **Antigravity** lo lee automáticamente al abrir el proyecto (soporte nativo de `AGENTS.md`).
- Cualquier otra herramienta compatible con el estándar `AGENTS.md` lo lee igual.

Si agregas una regla nueva, agrégala aquí — no dupliques contenido en `CLAUDE.md`.
